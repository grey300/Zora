...
function Course() {
  ...
  }, [router.query.courseId]); // Ensures the courseId is available before fetching

  const GetCourse = async () => {
    const result = await db
      .select()
      .from(CourseList)
      .where(eq(CourseList?.courseId, router.query.courseId)); // Access courseId from router.query
    setCourse(result[0]);
    setLoading(false);
  };
