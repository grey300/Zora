...
import OpenEndedPercentage from "@/app/dashboard/quiz/start-quiz/_components/OpenEndedPercentage"; // ✅
...
export default function OpenEnded({ game }) {
  ...
  const { mutate: endGame } = useMutation({
    mutationFn: async () => {
      const response = await axios.post(`/api/endGame`, { gameId: game.id });
    ...
    checkAnswer(undefined, {
      onSuccess: ({ percentageSimilar }) => {
        ...
        setAveragePercentage(
          (prev) => (prev + percentageSimilar) / (questionIndex + 1)
        );
  ...
  return (
    <div className="absolute -translate-x-1/2 -translate-y-1/2 md:w-[80vw] max-w-4xl w-[90vw] px-4 md:px-8 top-1/2 left-1/2">
