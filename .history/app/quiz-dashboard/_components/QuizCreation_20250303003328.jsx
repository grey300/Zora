...
function QuizCreation() {
  ...
  const { mutate: getQuestions, isLoading } = useMutation({
    mutationFn: async ({ amount, topic, type }) => {
      ...
      const response = await axios.post(
        "/dashboard/quiz/start-quiz/game", // Relative URL
        { amount, topic, type }
      );
  ...
  const onSubmit = async (input) => {
    ...
    generateQuiz(input); // Keep your existing logic

    // After generating the quiz, trigger getQuestions
    setShowLoader(true);

    // Now, call the mutation (getQuestions) with the form data
    getQuestions(input, {
      onError: (error) => {
        setShowLoader(false);
