import { auth } from "@clerk/nextjs/server"; // <-- Import this to get Clerk user
...
export async function POST(req) {
  try {
    const { userId } = auth(); // 👈 get real Clerk userId
    ...
    // Fetch quiz questions from Gemini API
    ...
    if (type === "mcq") {
      const manyData = data.questions.map((question) => {
        const options = [...question.options].sort(() => Math.random() - 0.5); // shuffle if you want
