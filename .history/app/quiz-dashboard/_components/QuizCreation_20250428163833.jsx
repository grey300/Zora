...
import { toast } from "@/components/ui/use-toast"; // ✅ You forgot to import toast, I added it!

function QuizCreation() {
  const [loading, setLoading] = useState(false);
  const router = useRouter();
  const { userId } = useAuth(); // ✅ get Clerk logged-in userId
  ...
  const { mutate: getQuestions, isLoading } = useMutation({
    mutationFn: async ({ amount, topic, type }) => {
      ...
      const response = await axios.post(
        `/api/game?userId=${userId}`, // ✅ Pass userId as query param
        { amount, topic, type },
        {
